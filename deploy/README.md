...
| `frontend-origin-certificate.pem`, `frontend-origin-private.key` | Certificado Origin CA e chave separados para `cep.lat` |
...
Na Cloudflare, configure SSL/TLS Full (strict), redirecionamento externo para HTTPS e ausência de cache/challenge interativo nas rotas da API. Use certificados Origin CA separados para `api.cep.lat` e `cep.lat`, permitindo rotacionar ou remover o front sem trocar a chave da API. Clientes acessam os domínios proxied. Na OCI/firewall, mantenha PostgreSQL fechado e restrinja SSH. Se depender das proteções da Cloudflare, restrinja a porta de origem às faixas dela após os testes administrativos.
