...
O exemplo de ambiente usa o Resend com TLS direto na porta 465; altere esses valores se trocar de provedor. O atualizador noturno detecta o GID do usuário não privilegiado na imagem e ajusta `security-code-hmac`, `smtp_username` e `smtp_password` para `root:<gid>` com modo `0640` antes de iniciar a janela de manutenção. Na primeira inicialização manual, aplique uma dessas duas estratégias antes de executar `migrate`.
